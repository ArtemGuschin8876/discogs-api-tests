...
test.describe('Discogs API - wantlist',  () => {
    ...
    test('Should add random release to want list', async ({authorizedClients, randomReleaseID}) => {
        const addResponse = await authorizedClients.wantlistClient.addReleaseToWantlist(`${Environment.USER_NAME}`, randomReleaseID)
