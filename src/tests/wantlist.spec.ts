...
test.describe('Discogs API - wantlist',  () => {
    ...
    test('Should return 200 and wantlist by username', async ({authorizedClients, userName}) => {
        const {responseBody} = await authorizedClients.wantlistClient.getWantlistByUsername(userName);
        WantlistAssertions.validateWantlistResponse(responseBody as WantlistResponse);
    });

    test('Should add random release to want list', async ({authorizedClients, randomReleaseID, userName, randomRelease, currentWantList}) => {
    ...
    test('Should delete release and return 204', async ({authorizedClients, addedReleaseID, userName, currentWantList}) => {
        await authorizedClients.wantlistClient.deleteReleaseFromWantList(userName, addedReleaseID);
