...
test.describe('Discogs API - Releases', () => {
    test('Should return 200, get release', async ({unathorizedClients, randomReleaseID, randomRelease}) => {
        ...
        ReleaseAssertions.validateCorrectResponseRelease(responseBody as ReleaseResponse, randomRelease, randomReleaseID);

    });

    test('Should return 200 and get release rating', async ({unathorizedClients, randomReleaseID}) => {
        
        const {responseBody} = await unathorizedClients.releaseClient.getReleaseRatingByReleaseId(randomReleaseID);
